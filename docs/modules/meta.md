...
Включает CSS `$module` так, как если бы он был написан непосредственно в содержимом этого миксина. CSS модуля будет включён, даже если ранее он уже был включён через другой вызов этой функции или правило `@use`.

Параметр `$module` может быть [значением модуля](../values/modules) или строкой, соответствующей пространству имён правила [`@use`](../at-rules/use) в текущем файле.
...
!!! tip "Совет"
    ...
    Поскольку таблица стилей `$module` вычисляется только один раз — при загрузке модуля, — все содержащиеся в ней [родительские селекторы](../style-rules/parent-selector) будут игнорировать всё, что находится за пределами этого модуля, даже если `meta.css()` включён внутри вложенного селектора.
...
Этот миксин является сокращённой записью для <code><a href="#css">meta.css</a>(<a href="#load">meta.load</a>($url, $with))</code>.
...
Если передан `$module`, вместо этого проверяется, содержит ли этот модуль указанную функцию. Параметр `$module` может быть [значением модуля](../values/modules) или строкой, соответствующей пространству имён правила [`@use`](../at-rules/use) в текущем файле.
...
Если `$module` имеет значение `null`, эта функция возвращает функцию с именем `$name` без пространства имён (включая [глобальные встроенные функции](../modules#global-functions)). В противном случае она возвращает функцию с именем `$name`, определённую в `$module`. Параметр `$module` может быть [значением модуля](../values/modules) или строкой, соответствующей пространству имён правила [`@use`](../at-rules/use) в текущем файле.

По умолчанию выдаёт ошибку, если `$name` не относится к функции Sass. Однако если `$css` равен `true`, вместо этого возвращает [обычную CSS-функцию](../at-rules/function/#plain-css-functions). Установка `$css: true` одновременно с передачей `$module` приводит к ошибке.
...
```scss
meta.get-mixin($name, $module: null) //=> миксин
```

Возвращает [значение миксина](../values/mixins) с именем `$name`.

Если `$module` имеет значение `null`, эта функция возвращает миксин с именем `$name` без пространства имён (включая [глобальные встроенные функции](../modules#global-functions)). В противном случае она возвращает миксин с именем `$name`, определённый в `$module`. Параметр `$module` может быть [значением модуля](../values/modules) или строкой, соответствующей пространству имён правила [`@use`](../at-rules/use) в текущем файле.

Вызывает ошибку, если `$name` не ссылается на миксин.
...
```scss
meta.get-module($module) //=> модуль
```

Возвращает [значение модуля](../values/modules) для `$module`. Параметр `$module` может быть [значением модуля](../values/modules) или строкой, соответствующей пространству имён правила [`@use`](../at-rules/use) в текущем файле.
...
Если `$module` имеет значение `null`, эта функция возвращает информацию о том, существует ли переменная с именем `$name` без пространства имён. В противном случае она возвращает информацию о том, определяет ли `$module` переменную с именем `$name`. Параметр `$module` может быть [значением модуля](../values/modules) или строкой, соответствующей пространству имён правила [`@use`](../at-rules/use) в текущем файле.
...
```scss
meta.load($url, $with: null) //=> модуль
```

Загружает [модуль](../at-rules/use) по адресу `$url` как [значение модуля](../values/module). Параметр `$with` задаёт [конфигурацию](../at-rules/use#configuration) модуля; если он передан, это должна быть карта, сопоставляющая имена переменных (без `$`) со значениями этих переменных, которые следует использовать в загружаемом модуле.

Если `$url` относительный, он интерпретируется относительно файла, в котором вызывается `meta.load()`.

Если модуль по адресу `$url` не был загружен до этого вызова, он немедленно вычисляется. Однако, в отличие от других способов загрузки модулей, при этом **не генерируется CSS**. Вместо этого можно включить миксин [`meta.css()`](#css), чтобы явно включить CSS модуля. CSS будет включён в том месте, где вызывается миксин, а не там, где загружается модуль.
...
* Не приводит к автоматическому выводу CSS загруженного модуля.
...
Возвращает, определен ли [миксин](../at-rules/mixin) с именем `$name`.

Если `$module` имеет значение `null`, эта функция возвращает информацию о том, существует ли миксин с именем `$name` без пространства имён. В противном случае она возвращает информацию о том, определяет ли `$module` миксин с именем `$name`. Параметр `$module` может быть [значением модуля](../values/modules) или строкой, соответствующей пространству имён правила [`@use`](../at-rules/use) в текущем файле.
...
Возвращает все функции, определённые в `$module`, в виде карты от имён функций к [значениям функций](../values/functions).
...
Возвращает все миксины, определённые в `$module`, в виде карты от имён миксинов к [значениям миксинов](../values/mixins).
...
Возвращает все переменные, определённые в `$module`, в виде карты от имён переменных (без `$`) к значениям этих переменных.

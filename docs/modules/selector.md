...
Функции этого модуля анализируют и изменяют селекторы. Возвращаемый селектор — это [список](../values/lists), разделённый запятыми (список селекторов), содержащий списки, разделённые пробелами (сложные селекторы), со [строками без кавычек](../values/strings#unquoted) внутри (комбинированные селекторы). Например, селектор `.main aside:hover, .sidebar p` будет возвращён как:
...
Возвращает, совпадает ли селектор `$super` со всеми элементами, которым соответствует селектор `$sub`.

Возвращает `true`, даже если `$super` соответствует *большему* количеству элементов, чем `$sub`.

...
Это работает даже при импорте модуля в общее пространство имён через `as *`. В таком случае присвоение значения переменной, объявленной в подключаемом модуле, приведёт к её перезаписи внутри него самого.
...
Прописывать абсолютные пути для каждой таблицы стилей было бы неудобно, поэтому алгоритм поиска модулей Sass упрощает эту задачу. Прежде всего, вам не нужно явно указывать расширение импортируемого файла: `@use "variables"` автоматически загрузит `variables.scss`, `variables.sass` или `variables.css`.

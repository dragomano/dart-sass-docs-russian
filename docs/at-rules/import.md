...
!!! warning "Внимание"
    ...
    * Нет способа определить приватные элементы или селекторы-заполнители, недоступные для последующих таблиц стилей.

    Новая модульная система и правило `@use` решают все эти проблемы.

    <h4>Как мигрировать?</h4>
...
Когда Sass импортирует файл, этот файл вычисляется так, как если бы его содержимое находилось непосредственно на месте `@import`. Любые [миксины](../at-rules/mixin), [функции](../at-rules/function) и [переменные](../variables) из импортированного файла становятся доступными, а весь его CSS включается точно в то место, где был написан `@import`. Более того, любые миксины, функции или переменные, определённые до `@import` (включая те, что были получены из других `@import`), доступны в импортированной таблице стилей.
...
Чтобы [настроить модуль](../at-rules/use#configuration), который вы подключаете через `@import`, нужно объявить глобальные переменные до того, как вы в первый раз вызовете этот `@import`.

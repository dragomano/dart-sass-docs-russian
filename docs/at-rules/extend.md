...
Наследовать можно только *простые селекторы* — одиночные селекторы вроде `.info` или `a`. Если бы можно было наследовать составной селектор (например, `.message.info`), то по правилам `@extend` элементы, использующие наследование, должны были бы стилизоваться так, будто они соответствуют и `.message`, и `.info` одновременно. Но это ничем не отличается от ситуации, когда вы просто пишете `@extend .message, .info`. Поэтому в поддержке наследования именно связки `.message.info` нет никакого практического смысла.

Аналогично, если бы можно было наследовать сложный селектор (например, `.main .info`), это дало бы (почти) тот же результат, что и обычное наследование `.info`. Те незначительные отличия, которые могли бы возникнуть, не стоят путаницы: такой код выглядел бы так, будто он делает нечто существенно иное. Поэтому это тоже запрещено.
...
Хотя использовать `@extend` внутри [`@media` и других CSS-директив](../at-rules/css) можно, запрещено наследовать селекторы, которые находятся за пределами этого блока (снаружи `@media`).

Это связано с тем, что наследующий селектор должен работать только в рамках конкретного медиа-условия (например, при определённой ширине экрана). Не существует способа сохранить это ограничение в итоговом CSS-файле без полного дублирования всех стилей наследуемого правила. Поэтому, чтобы избежать раздувания кода и логических ошибок, такие действия запрещены.

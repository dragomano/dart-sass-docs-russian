...
Вам не нужно писать весь ваш Sass в одном файле. Вы можете разделить его как угодно с помощью правила `@use`. Это правило загружает другой файл Sass как *модуль*, что означает, что вы можете обращаться к его переменным, [миксинам](#mixins) и [функциям](#functions) в вашем файле Sass с пространством имён, основанным на имени файла. Использование файла также включит CSS, который он генерирует, в ваш скомпилированный вывод!

...
Самый прямой способ сделать это — в терминале. После установки Sass вы можете компилировать ваш Sass в CSS с помощью команды `sass`. Вам нужно указать Sass, с каким файлом работать и куда выводить CSS. Например, выполнение `sass input.scss output.css` в терминале возьмёт один файл Sass, `input.scss`, и скомпилирует его в `output.css`.

Вы также можете отслеживать отдельные файлы или каталоги с помощью флага `--watch`. Флаг watch указывает Sass следить за изменениями в исходных файлах и повторно компилировать CSS каждый раз, когда вы сохраняете свой Sass. Если вы хотите отслеживать (вместо ручной сборки) ваш файл `input.scss`, вам просто нужно добавить флаг `--watch` к вашей команде следующим образом:
...
Sass позволяет вкладывать CSS-селекторы друг в друга в соответствии с визуальной иерархией вашего HTML. При этом стоит помнить: слишком глубокая вложенность приводит к избыточно специфичному CSS, который трудно поддерживать и обычно считается плохой практикой.
...
Вам не нужно писать весь ваш Sass в одном файле. Вы можете разделить его как угодно с помощью правила `@use`. Это правило загружает другой файл Sass как *модуль*, что означает, что вы можете обращаться к его [переменным](#variables), [миксинам](#mixins) и [функциям](../at-rules/functions) в вашем файле Sass с пространством имён, основанным на имени файла. Использование файла также добавит CSS, который он генерирует, в ваш скомпилированный вывод!

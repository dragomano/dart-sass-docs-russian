...
Чтобы загрузить модуль с настройками, пишите `@use <url> with (<переменная>: <значение>, <переменная>: <значение>)`. Настроенные значения переопределят изначальные значения переменных. Настраивать можно только переменные, написанные на верхнем уровне таблицы стилей с флагом `!default`.
...
Переменные, объявленные на верхнем уровне таблицы стилей, являются *глобальными*. Это значит, что к ним можно обращаться из любого места в этом модуле после их объявления. Однако это правило применимо не ко всем переменным. Те из них, что объявлены внутри блоков (в фигурных скобках в SCSS или с помощью отступов в Sass), обычно являются *локальными*. Такие переменные доступны только в пределах того блока, в котором они были определены.
...
Локальную переменную можно назвать так же, как и глобальную. В этом случае в коде будут существовать две разные переменные с одинаковым именем: одна локальная, а другая — глобальная. Это сделано для безопасности: разработчик, создавая локальную переменную, не сможет случайно перезаписать значение глобальной переменной, о существовании которой он может даже не догадываться.

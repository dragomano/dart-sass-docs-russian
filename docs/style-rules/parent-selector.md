...
> Родительский селектор, `&`, — это специальный селектор, изобретённый Sass, который используется во [вложенных селекторах](../style-rules/#nesting) для ссылки на внешний селектор. Он позволяет повторно использовать внешний селектор более сложными способами, например добавляя [псевдокласс](https://developer.mozilla.org/en-US/docs/Web/CSS/Pseudo-classes) или добавляя селектор *перед* родительским.
...
## Без вложенности {#without-nesting}

Точно так же, как в обычном CSS, вы можете использовать селектор родителя (`&`) вне вложенного правила. В этом случае он выводится как обычный CSS, который будет обрабатываться браузером, и интерпретируется им как [корень области видимости](https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/Selectors/Nesting_selector#using_outside_nested_rule).

!!! note "Примечание"

    Вы не можете добавлять суффикс к селекторам родителя, которые не находятся внутри вложенного правила, поскольку не к чему применять этот суффикс.
...
Родительский селектор также может использоваться в SassScript. Это специальное выражение, возвращающее текущий родительский селектор в формате, принятом в [функциях селекторов](../modules/selector#selector-values). Оно представляет собой список через запятую (список селекторов), состоящий из списков через пробел (сложные селекторы), включающих в себя строки без кавычек (составные селекторы).

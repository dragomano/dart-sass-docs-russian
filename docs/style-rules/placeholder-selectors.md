...
> В Sass существует особый вид селектора — «заполнитель». Он напоминает селектор класса, но начинается с `%` и не выводится в итоговый CSS. Фактически любой сложный селектор, *содержащий* заполнитель, исключается из компиляции. Это же касается и правил стиля, все селекторы которых включают в себя такие элементы.
